...
const App = () => {
  ...
  const [activeTab] = useState<'single' | 'blogger' | 'profile'>(
    getInitialTab()
  )
  ...
  const [benchmark] = useState<number>(3)
  ...
  const syncItems = async (items: CollectedItem[]) => {
    ...
    const MAX_BATCH_BYTES = 80 * 1024 // Keep under Express default (100 KB) to avoid 413
  ...
  if (!isOpen) {
    return (
      <div
        className="fixed top-24 right-10 z-[2147483647] cursor-pointer group"
        onClick={() => {
          setIsOpen(true)
        }}
      >
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#ff2442] text-white shadow-[0_15px_35px_rgba(255,36,66,0.35)] border border-white/20 transition-transform group-hover:scale-105 active:scale-95">
          ...
          <span className="text-sm font-semibold tracking-wide whitespace-nowrap">薯小智助手</span>
        </div>
      </div>
    )
  }
          ...
          <>
            ...
            <div className="bg-white px-4 pt-4 pb-2 flex flex-col gap-4 shadow-sm relative z-10 flex-shrink-0">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="w-9 h-9 flex items-center justify-center rounded-full border border-[#ff2442]/30 bg-[#ff2442]/10 text-[#ff2442] shadow-sm hover:bg-[#ff2442] hover:text-white transition-colors transition-transform hover:-translate-y-0.5"
                >
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex-shrink-0">
                  <button
                    ...
                    className={`w-full font-bold py-3 rounded-2xl shadow-xl flex items-center justify-center gap-2 transition-all transform ${isCollecting || isSyncing
                      ...
                      : 'bg-yellow-400 hover:bg-yellow-500/90 text-gray-900 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer'
                      }`}
                  >
                    {isSyncing ? (
                      <>
