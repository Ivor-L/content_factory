...
3.  **数据库连接**：应用现在直接连接云端 Supabase (`db.<project-ref>.supabase.co:5432`)，无需再暴露本地 54322 端口，也无需共存 Supabase 容器。
...
### 数据库连接失败
如果 Docker 容器无法连接数据库，请确认 `.env` 中的 `DATABASE_URL` 指向云端 Supabase，并且服务器可以访问外部 `db.<project-ref>.supabase.co:5432`（必要时开放防火墙 egress）。
