...
3.  **54322 端口** (`supabase_db`)：这是 Supabase Postgres 的对外映射（宿主机 127.0.0.1:54322 → 容器 5432），我们的应用需要直连它，且必须禁用 SSL。
...
### 数据库连接失败
如果 Docker 容器无法连接数据库，请检查防火墙设置，确保 54322 端口允许访问，或者直接通过 Docker 网络别名连接 `supabase_db:5432`。
