...
async function requestJson(url: string, init: RequestInit = {}, runtime?: Awaited<ReturnType<typeof resolveRuntime>>) {
  ...
  if (!res.ok) {
    const payload = typeof data === 'object' && data !== null ? data as Record<string, unknown> : { message: String(data || '') };
    ...
    (err as Error & { status?: number; code?: string; payload?: unknown; explanation?: ReturnType<typeof explainError> }).status = res.status;
    (err as Error & { status?: number; code?: string; payload?: unknown; explanation?: ReturnType<typeof explainError> }).code = code;
    (err as Error & { status?: number; code?: string; payload?: unknown; explanation?: ReturnType<typeof explainError> }).payload = payload;
    (err as Error & { status?: number; code?: string; payload?: unknown; explanation?: ReturnType<typeof explainError> }).explanation = explanation;
    throw err;
  }
  return data;
}
...
function explainError(code: unknown, message?: unknown): { code: string; title: string; message: string; nextActions: string[] } {
  const normalized = String(code || '').toLowerCase();
  const raw = String(message || '').trim();
  const map: Record<string, { title: string; message: string; nextActions: string[] }> = {
    unauthorized: {
      title: '认证失败',
...
function withExplanation<T extends Record<string, any>>(data: T): T {
...
async function doctor(flags: Record<string, string | boolean>) {
  ...
  await check('capability_list', async () => {
    const data = await requestJson(`${runtime.apiBaseUrl}/api/agent/capabilities`, {}, runtime) as { capabilities?: unknown[] };
  ...
  await check('stored_nexTide_api_key', async () => {
    if (!runtime.userApiKey) throw new Error('No NexTide API Key found. Run nextide auth login.');
    return { present: true };
  });
  await check('capability_environment_metadata', async () => {
    const data = await requestJson(`${runtime.apiBaseUrl}/api/agent/capabilities`, {}, runtime) as { capabilities?: Array<{ id: string; requiredEnv?: string[] }> };
  ...
  await check('capability_credit_config_audit', async () => {
    const data = await requestJson(`${runtime.apiBaseUrl}/api/agent/capabilities?includeCreditAudit=1`, {}, runtime) as {
      capabilities?: Array<{ id: string; featureKey?: string }>;
      creditAudit?: {
        ok?: boolean;
        total?: number;
        okCount?: number;
        missingFeatureKey?: unknown[];
        missingCreditConfig?: unknown[];
        disabledCreditConfig?: unknown[];
      };
    };
    if (data.creditAudit) {
...
async function capabilityExample(args: string[], flags: Record<string, string | boolean>) {
  const id = args[2];
  if (!id) throw new Error('capability id is required');
  const runtime = await resolveRuntime(flags);
  const data = await requestJson(`${runtime.apiBaseUrl}/api/agent/capabilities`, {}, runtime) as { capabilities?: Array<Record<string, any>> };
...
function buildInputSkeleton(schema: Record<string, any>): Record<string, unknown> {
  ...
  for (const [key, field] of Object.entries(schema)) {
    ...
    if (field.type === 'number') input[key] = 0;
    else if (field.type === 'boolean') input[key] = false;
    else if (field.type === 'object') input[key] = {};
    else if (String(field.type).endsWith('[]') || field.type === 'array') input[key] = [];
    else input[key] = `TODO_${key}`;
  }
  return input;
}
...
async function capabilityRun(args: string[], flags: Record<string, string | boolean>) {
  ...
  }, runtime) as { run?: { runId?: string; status?: string } };
...
async function runWait(args: string[], flags: Record<string, string | boolean>) {
  const id = args[2];
  if (!id) throw new Error('run id is required');
  const runtime = await resolveRuntime(flags);
...
async function runFollow(args: string[], flags: Record<string, string | boolean>) {
  const id = args[2];
  if (!id) throw new Error('run id is required');
  const runtime = await resolveRuntime(flags);
  const outputDir = valueFlag(flags, 'output-dir') || path.join('.nextide', 'output', id);
  const data = await pollRun(runtime, id, flags, { log: true });
  const status = (data as { run?: { status?: string } })?.run?.status || (data as { status?: string })?.status;
  const shouldExport = flags.artifacts !== false && status === 'succeeded';
  let artifactExport: unknown = undefined;
  if (shouldExport) {
  ...
  const explanation = status !== 'succeeded'
    ? explainError((data as { error?: { code?: string }, run?: { error?: { code?: string } } })?.error?.code || (data as { run?: { error?: { code?: string } } })?.run?.error?.code || status, (data as { error?: { message?: string }, run?: { error?: { message?: string } } })?.error?.message || (data as { run?: { error?: { message?: string } } })?.run?.error?.message)
    : undefined;
...
async function pollRun(runtime: Awaited<ReturnType<typeof resolveRuntime>>, runId: string, flags: Record<string, string | boolean>, options: { log?: boolean } = {}) {
  ...
  while (Date.now() <= deadline) {
    last = await requestJson(`${runtime.apiBaseUrl}/api/agent/runs/${encodeURIComponent(runId)}`, {}, runtime) as { run?: { status?: string } };
    const status = (last as { run?: { status?: string } }).run?.status;
  ...
  return withExplanation({
    run: (last as { run?: unknown } | null)?.run,
    error: {
      code: 'wait_timeout',
...
async function runCancel(args: string[], flags: Record<string, string | boolean>) {
  const id = args[2];
  if (!id) throw new Error('run id is required');
  const runtime = await resolveRuntime(flags);
...
async function runArtifacts(args: string[], flags: Record<string, string | boolean>) {
  const id = args[2];
  if (!id) throw new Error('run id is required');
  const runtime = await resolveRuntime(flags);
  const outputDir = valueFlag(flags, 'output-dir') || path.join('.nextide', 'output', id);
...
async function exportArtifactsForRun(id: string, runtime: Awaited<ReturnType<typeof resolveRuntime>>, outputDir: string, flags: Record<string, string | boolean>) {
  ...
  const data = await requestJson(`${runtime.apiBaseUrl}/api/agent/runs/${encodeURIComponent(id)}/result`, {}, runtime) as Record<string, any>;
...
function defaultArtifactName(artifact: Record<string, any>, index: number): string {
  ...
  if (type === 'image') return `image-${index + 1}${extensionFromMime(artifact.mimeType) || '.png'}`;
  if (type === 'video') return `video-${index + 1}${extensionFromMime(artifact.mimeType) || '.mp4'}`;
  if (type === 'audio') return `audio-${index + 1}${extensionFromMime(artifact.mimeType) || '.mp3'}`;
  if (type === 'text') return `text-${index + 1}.txt`;
...
async function downloadArtifact(url: string, localPath: string): Promise<{ ok: true }> {
...
function buildRecommendedMessage(input: { title: string; runStatus: string; imageCount: number; videoCount: number; audioCount: number; hasTable: boolean }): string {
  ...
  if (input.imageCount) pieces.push(`图片：${input.imageCount} 张`);
  if (input.videoCount) pieces.push(`视频：${input.videoCount} 个`);
  if (input.audioCount) pieces.push(`音频：${input.audioCount} 个`);
  if (input.hasTable) pieces.push('已生成数据表');
  return pieces.join(' · ');
}
...
type DatatableSpec = { title: string; columns: Array<{ key: string; label: string; type: string }>; rows: Array<Record<string, unknown>> };
...
function buildDatatable(input: {
  ...
  if (specialized.rows.length > 0) return specialized;
...
function buildSpecializedDatatable(capabilityId: string, data: Record<string, any>, artifacts: Array<Record<string, unknown>>): DatatableSpec {
  if (capabilityId === 'xhs.note.collect') return buildXhsNoteCollectTable(data);
  if (capabilityId.startsWith('social.')) return buildSocialCollectTable(data, artifacts, capabilityId);
  if (capabilityId === 'product.selling_point.analysis') return buildProductAnalysisTable(data);
  if (capabilityId === 'viral.breakdown.video_prompts') return buildViralBreakdownPromptTable(data, artifacts);
  if (capabilityId === 'content.wechat.longform.write') return buildWechatLongformTable(data, artifacts);
  if (capabilityId === 'xhs.infographic.generate') return buildInfographicTable(data, artifacts, capabilityId);
  if (['digital-human.video.generate', 'motion.replication.image_to_video', 'viral.midform.video.generate'].includes(capabilityId)) return buildVideoTaskTable(data, artifacts, capabilityId);
  return { title: humanizeTitle(capabilityId), columns: [], rows: [] };
}

function buildXhsNoteCollectTable(data: Record<string, any>): DatatableSpec {
  const rawItems = pickFirstArray(
    data?.result?.items,
    data?.result?.data?.items,
    data?.result?.data?.savedReferences,
    data?.result?.savedReferences,
    data?.result?.data,
  );
...
function buildSocialCollectTable(data: Record<string, any>, artifacts: Array<Record<string, unknown>>, capabilityId: string): DatatableSpec {
...
function buildWechatLongformTable(data: Record<string, any>, artifacts: Array<Record<string, unknown>>): DatatableSpec {
  const result = asPlainRecord(data?.result?.data || data?.result || {});
  const article = String(
    pickValue(result, ['article', 'content', 'markdown', 'text', 'body']) ||
...
function buildInfographicTable(data: Record<string, any>, artifacts: Array<Record<string, unknown>>, capabilityId: string): DatatableSpec {
  const result = asPlainRecord(data?.result?.data || data?.result || {});
  const run = asPlainRecord(data?.run || {});
...
function buildVideoTaskTable(data: Record<string, any>, artifacts: Array<Record<string, unknown>>, capabilityId: string): DatatableSpec {
  const result = asPlainRecord(data?.result?.data || data?.result || {});
  const run = asPlainRecord(data?.run || {});
  const business = asPlainRecord(data?.business || {});
...
function buildProductAnalysisTable(data: Record<string, any>): DatatableSpec {
  ...
  if (detailed) rows.push(compactRow({ type: '总结', content: detailed, source: 'analysis', order: rows.length + 1 }));
  return {
    title: '产品卖点分析',
    columns: columnsForKeys([
      ['type', '类型', 'badge'],
...
function columnsForKeys(defs: Array<[string, string, string]>, rows: Array<Record<string, unknown>>): Array<{ key: string; label: string; type: string }> {
...
function platformFromCapability(capabilityId: string): string {
  if (capabilityId.includes('tiktok')) return 'tiktok';
  if (capabilityId.includes('instagram')) return 'instagram';
  if (capabilityId.includes('facebook')) return 'facebook';
  if (capabilityId.includes('comments')) return 'comments';
  return 'social';
}

function splitArticleIntoSections(article: string): Array<{ heading: string; content: string }> {
...
function countCjkWords(text: string): number {
...
function inferArticleSectionRole(heading: string, index: number, total: number): string {
  const lower = heading.toLowerCase();
  if (index === 0 || /开头|引言|导语|前言|hook/.test(lower)) return '开头';
  if (index === total - 1 || /结尾|总结|行动|购买|报名|联系|cta/.test(lower)) return '收束/转化';
  if (/案例|故事|经历|场景/.test(lower)) return '案例';
  if (/方法|步骤|怎么|策略|清单/.test(lower)) return '方法';
  if (/为什么|原因|问题|痛点/.test(lower)) return '论证';
  return '正文';
}

function inferCallToAction(content: string): string | undefined {
  const match = content.match(/([^。！？\n]*(关注|私信|评论|购买|报名|预约|扫码|点击|领取|下载)[^。！？\n]*[。！？]?)/);
  return match?.[1]?.trim();
}

function firstArtifactMetadataValue(artifacts: Array<Record<string, unknown>>, keys: string[]): unknown {
...
function normalizeRows(items: unknown[]): Array<Record<string, unknown>> {
  return items
    .map((item) => typeof item === 'object' && item !== null ? flattenRow(item as Record<string, unknown>) : { value: item })
    .filter((row) => Object.keys(row).length > 0)
    .slice(0, 1000);
}

function flattenRow(record: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (Array.isArray(value)) {
      ...
      if (nestedKeys.length <= 6 && nestedKeys.every((nestedKey) => typeof nested[nestedKey] !== 'object')) {
        for (const nestedKey of nestedKeys) row[`${key}.${nestedKey}`] = nested[nestedKey];
      } else {
...
function inferColumns(rows: Array<Record<string, unknown>>): Array<{ key: string; label: string; type: string }> {
  const keys: string[] = [];
...
function inferColumnType(key: string, values: unknown[]): string {
  ...
  if (present.every((value) => typeof value === 'number')) return lower.includes('rate') || lower.includes('percent') ? 'percent' : 'number';
  if (lower.includes('status') || lower.includes('type') || lower.includes('category') || lower.includes('platform')) return 'badge';
...
function renderArtifactCard(artifact: Record<string, unknown>, index: number): string {
  const type = String(artifact.type || 'artifact');
  if (type === 'image') return renderImageFigure(artifact, index);
  if (type === 'video') return renderVideoFigure(artifact, index);
  if (type === 'audio') return renderAudioCard(artifact, index);
  return renderDataCard(artifact, index);
}

function artifactSrc(artifact: Record<string, unknown>): string {
...
function artifactCaption(artifact: Record<string, unknown>, fallback: string): string {
...
function renderImageFigure(artifact: Record<string, unknown>, index: number): string {
...
function renderVideoFigure(artifact: Record<string, unknown>, index: number): string {
  const src = artifactSrc(artifact);
...
function renderAudioCard(artifact: Record<string, unknown>, index: number): string {
  const src = artifactSrc(artifact);
...
function renderDataCard(artifact: Record<string, unknown>, index: number): string {
...
function safeArtifactName(value: unknown): string {
...
async function main() {
  ...
  if (cmd === 'doctor') return doctor(parsed.flags);
  if (cmd === 'status') {
  ...
  if (cmd === 'capability' && sub === 'example') return capabilityExample(parsed.args, parsed.flags);
  ...
  if (cmd === 'run' && sub === 'wait') return runWait(parsed.args, parsed.flags);
  if (cmd === 'run' && sub === 'follow') return runFollow(parsed.args, parsed.flags);
  if (cmd === 'run' && sub === 'cancel') return runCancel(parsed.args, parsed.flags);
  if (cmd === 'run' && sub === 'result') return runResult(parsed.args, parsed.flags);
  if (cmd === 'run' && sub === 'artifacts') return runArtifacts(parsed.args, parsed.flags);
  throw new Error(`Unknown command: ${parsed.args.join(' ')}`);
}

main().catch((error) => {
  const rich = error as Error & { explanation?: ReturnType<typeof explainError>; code?: string; payload?: unknown };
  if (rich.explanation) {
