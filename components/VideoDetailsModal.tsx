...
export function VideoDetailsModal({ item, onClose }: VideoDetailsModalProps) {
  ...
  const model = resultData?.model || "Veo3.1 Fast";
  const ratio = resultData?.ratio || "9:16";
  const duration = resultData?.duration || "8s";
  const language = resultData?.language || "EN";
  const credits = resultData?.credits || "20";
  const createdAt = new Date(item.createdAt).toLocaleDateString();
  
  const scene1Text = script.breakdown ? JSON.parse(script.breakdown)[0]?.visual || "Scene 1 visual description..." : "No script breakdown available.";
  const spokenText = script.breakdown ? JSON.parse(script.breakdown)[0]?.audio || "No audio script available." : "No script audio available.";
  const promptText = resultData?.prompt || "A confident young man, appearing to be in his early 20s, with short brown hair and striking green eyes, wearing a dark casual shirt. Context: A modern, clean kitchen with white countertops and light-colored walls, suggesting a home setting.";
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white dark:bg-gray-900">
          ...
          <div className="flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700">
              ...
              <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                ...
                <div>
                  <div className="flex items-center gap-2 text-gray-400 text-[10px] uppercase font-bold mb-1">
                    <Coins size={12} /> {t.replication.credits}
                  </div>
                  <div className="font-bold text-gray-900 dark:text-white text-sm">{credits}</div>
                </div>
                <div>
                  <div className="flex items-center gap-2 text-gray-400 text-[10px] uppercase font-bold mb-1">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 flex-1">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-6">{t.replication.aiPrompts}</h3>
      ...
      {/* Footer Actions */}
      <div className="p-6 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 flex justify-end items-center gap-3 shrink-0">
        <div className="flex gap-2 mr-auto">
            <button className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-medium transition-colors text-gray-700 dark:text-gray-300">
                <ThumbsUp size={16} /> {t.replication.good}
            </button>
            <button className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-medium transition-colors text-gray-700 dark:text-gray-300">
                <ThumbsDown size={16} /> {t.replication.bad}
            </button>
        </div>
