...
## Advanced Database Write Patterns

以下 5 个模式从实战工作流中提炼，覆盖数据清洗、防空写入、图片存储和 Schema 适配。

### Array Dedup + Truncation（数组去重 + 截断）

**Use Case**: 写入 Notion/Airtable 时，数组数据需要去重且不超过字段长度限制。

**Notion 限制**：Rich Text 属性最大 2000 字符。
...
return items.map(item => {
  ...
  for (const tag of tags) {
    const newLength = totalLength + tag.length + (result.length > 0 ? 2 : 0); // +2 for ", "
    if (newLength > MAX_LENGTH) break;
    result.push(tag);
    totalLength = newLength;
  }

  return { json: { ...item.json, tags_clean: result.join(', ') } };
});
```

**Notion 节点写入**：
- property type: `rich_text`
- value: `={{ $json.tags_clean }}`

---

### Variant Flattening（变体展平）

**Use Case**: 电商数据中嵌套的变体数组（颜色、尺码）需要展平为一维字符串写入数据库。

**输入数据结构**：
```json
{
  "variants": [
    { "color": "红色", "size": "M", "price": 99 },
    { "color": "蓝色", "size": "L", "price": 109 }
  ]
}
```
...
const items = $input.all();
return items.map(item => {
  const variants = item.json.variants || [];

  // 展平为描述字符串
  const variantText = variants
    .map(v => `${v.color}/${v.size}: ¥${v.price}`)
    .join(' | ');
...
### Empty Data Guard with ignoreIfEmpty（ignoreIfEmpty 防空写入）

**Use Case**: 上游无数据时跳过数据库写入，避免空记录或报错。
...
const items = $input.all().filter(item => {
  const d = item.json;
  return d.title && d.title.trim() !== '';  // 至少标题不为空
});

if (items.length === 0) {
  return [{ json: { skipped: true, reason: 'no valid items' } }];
}
return items;
```
...
### Image URL Extraction + Storage（图片 URL 提取与存储）

**Use Case**: 从 HTML 或 API 响应中提取图片 URL，存入数据库对应字段。
...
return items.map(item => {
  ...
  const apiImages = item.json.images || [];

  // 从 HTML 提取 img src
  const htmlImages = [...html.matchAll(/<img[^>]+src="([^"]+)"/g)]
    .map(m => m[1])
    .filter(url => !url.includes('placeholder') && !url.includes('1x1'));

  // 合并 + 去重
  const allImages = [...new Set([...apiImages, ...htmlImages])];

  // 取主图（第一张）和缩略图列表
  const mainImage = allImages[0] || '';
  const thumbnails = allImages.slice(0, 5);  // 最多 5 张

  return {
    json: {
      ...item.json,
      main_image_url: mainImage,
      thumbnail_urls: thumbnails.join('\n'),  // Notion: 换行分隔
...
**Notion 写入**：
- main_image_url → Files & Media 属性（type: `url`）
- thumbnail_urls → Rich Text 属性

**Airtable 写入**：
- main_image_url → Attachment 字段（需 URL 格式）
- thumbnail_urls → Long Text 字段

---

### Airtable Schema + defaultMatch（Airtable Schema 适配）

**Use Case**: Airtable 写入时的 Schema 映射和更新匹配配置。

**Airtable 节点配置（typeVersion: 2）**：
```javascript
{
  operation: "upsert",      // create | update | upsert
  table: { value: "tblXXXX", mode: "id" },
  columns: {
    ...
    value: {
      ...
      "URL": "={{ $json.url }}",
      "Price": "={{ $json.price }}",
      "Tags": "={{ $json.tags_clean }}",
      "Status": "active"
    }
  },
  options: {
    bulkSize: 10,           // 批量写入大小
    typecast: true           // 自动类型转换（字符串→数字/日期）
  }
}
```

**Upsert 的 defaultMatch 配置**：
```javascript
{
  operation: "upsert",
  columns: {
    matchingColumns: ["URL"],  // 用 URL 字段匹配去重
    value: { ... }
  }
}
```
...
| Airtable 类型 | n8n 表达式 | 注意事项 |
|---|---|---|
| Single Line Text | 直接字符串 | 最大 100K 字符 |
| Long Text | 支持 Markdown | 无限制 |
| Number | 确保是数字类型 | `={{ Number($json.price) }}` |
| Single Select | 值必须匹配已有选项 | 或设 typecast: true |
| Multiple Select | 逗号分隔字符串 | `={{ $json.tags.join(',') }}` |
| Date | ISO 格式 | `={{ $json.date }}` (yyyy-MM-dd) |
| Checkbox | boolean | `={{ $json.active === true }}` |
| URL | 完整 URL | 含 https:// |
| Attachment | URL 数组 | `={{ [{ url: $json.image_url }] }}` |

**最佳实践**：
- typecast: true 允许自动类型转换
- 用 upsert + matchingColumns 防止重复写入
- bulkSize: 10 批量写入提高性能
- 不要跳过字段映射依赖自动匹配
