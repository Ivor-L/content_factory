...
## ⚠️ n8n 2.0 Breaking Change: Python Runtime

n8n 2.0 **移除了内置 Pyodide 运行时**。Python Code 节点（Code v2）仍声明支持 Python 语言选项，但实际执行需要配置独立的 **Task Runner** 环境。

- **无 Task Runner**：Python 代码无法执行，节点报错
- **需要配置**：`N8N_RUNNERS_ENABLED=true` + 独立 Task Runner 进程
- **建议**：优先使用 JavaScript，除非已确认实例配置了 Python Task Runner
