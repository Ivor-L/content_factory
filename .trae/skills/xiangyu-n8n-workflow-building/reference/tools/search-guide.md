...
## Official Docs WebFetch Strategy

When MCP `get_node` results are insufficient, fetch official documentation directly:

```
Step 1: Grep nodes-catalog.md for the node → extract [docs] link
Step 2: WebFetch(url=docs_link, prompt="Extract operations, required parameters, authentication, known issues")
Step 3: Use extracted info to configure node
```

**When to use**:
- Step 04: Proactive fetch for core [L2] nodes before building
- Step 06: Fallback when `get_node(detail="standard")` lacks parameter details
- Step 08: Primary strategy (Strategy 0) for validation error resolution

**Rules**:
- Max 3 node docs per step to avoid token waste
- Skip [L3] nodes (reference/ already has deep knowledge)
- Cache: if fetched in Step 04, reuse in Step 06/08 without re-fetching
