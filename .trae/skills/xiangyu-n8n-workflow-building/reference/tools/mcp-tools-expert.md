...
### Mistake 2: Using Outdated typeVersion

**Problem**: Reference docs 中的 typeVersion 可能落后于 n8n 实例实际支持的版本。MCP `get_node` 返回的 `defaultVersion` 是实例支持的**最新稳定版本**，可直接使用。

```javascript
// ✅ CORRECT - MCP get_node 返回的 defaultVersion 可信
get_node({nodeType: "nodes-base.formTrigger"})
// Returns defaultVersion: 2.5 → 直接使用 typeVersion: 2.5

// ❌ WRONG - 使用过时的硬编码版本
// typeVersion: 2.3  // 旧版本，缺少 fieldName 分离等功能
```

**Rule**: 使用 `get_node` 返回的 `defaultVersion` 作为 typeVersion。升级 n8n 实例后，需同步检查并更新 reference 文档中的版本号。

**当前 n8n 2.x 节点版本对照表**：

| 节点 | nodeType | defaultVersion |
|------|----------|----------------|
| Form Trigger | `formTrigger` | **2.5** |
| Form (Ending) | `form` | **2.5** |
| HTTP Request | `httpRequest` | **4.4** |
| IF | `if` | **2.3** |
| Switch | `switch` | **3.4** |
| Code | `code` | **2** |
| Webhook | `webhook` | **2.1** |
| Set | `set` | **3.4** |
| Merge | `merge` | **3.2** |
| Schedule Trigger | `scheduleTrigger` | **1.3** |
| Execute Workflow | `executeWorkflow` | **1.3** |
| Respond to Webhook | `respondToWebhook` | **1.5** |

### Mistake 3: Using detail="full" by Default
