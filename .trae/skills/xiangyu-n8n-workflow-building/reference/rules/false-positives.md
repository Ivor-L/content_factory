...
**Status**: False positive for IF v2.3+（Filter v3 引擎）

**Why it occurs**: Auto-sanitization adds metadata, but validation runs before sanitization。IF v2.3 使用 Filter v3 条件引擎，metadata 格式已更新。
...
**Status**: False positive when using "fallback" mode（Switch v3.4）

**Why it occurs**: Fallback creates extra output。Switch v3.3+ 支持动态表达式控制输出数量。
...
## Async Polling False Positives

异步轮询模式中的误报场景：

### FP-POLL-001: Wait Node "Missing Connection" Warning

**误报内容**: 验证报告 Wait 节点后缺少连接
**实际情况**: Wait 节点的 resume 机制会在等待结束后继续执行，连接到后续轮询节点是正确的
**判断方式**: 检查 Wait → HTTP Request → IF 完整链路是否存在

### FP-POLL-002: "Circular Connection" in Polling Loop

**误报内容**: 检测到循环连接（IF False → Wait → HTTP → IF）
**实际情况**: 异步轮询天然就是循环结构，这是预期行为
**判断方式**: 确认循环有明确的退出条件（IF True 分支 + maxRetries 计数器）

### FP-POLL-003: "Unused Node Output" on Wait

**误报内容**: Wait 节点输出未被使用
**实际情况**: Wait 节点在 timeInterval 模式下，到时间后自动继续执行下游节点
**判断方式**: 确认 Wait 节点后有连接到 HTTP Request 或其他处理节点
