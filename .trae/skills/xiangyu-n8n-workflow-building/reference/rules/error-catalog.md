...
### E-BATCH-001: Infinite Loop

**症状**: SplitInBatches 节点永不停止
**原因**: `options.reset` 设为 true，或 Loop 输出连回了错误位置
**修复**:
- 确认 `options.reset: false`
- Loop 输出连接到处理链路（非 SplitInBatches 自身的输入）

### E-BATCH-002: Aggregate Position Error

**症状**: 报错 "Items have the same input source"
**原因**: Aggregate 节点放在循环内部
**修复**: 将 Aggregate 移到 SplitInBatches 的 Done 输出之后

### E-BATCH-003: Empty Batch

**症状**: 批处理输入为空数组时工作流报错
**原因**: 未检查输入数据是否为空
**修复**: 在 SplitInBatches 前加 IF 节点检查 `{{ $json.items?.length > 0 }}`

### E-BATCH-004: Batch Size Too Large

**症状**: API 返回 429 Too Many Requests
**原因**: batchSize 超过 API 限速
**修复**: 减小 batchSize，或在循环内加 Wait 节点
