...
## Advanced Luxon Date Operations

n8n 内置 Luxon 库，通过 `$now` 和 `$today` 提供日期操作。

### Date Arithmetic

**加减天数**：
```
={{ $today.plus({ days: 7 }).toISODate() }}          // 7 天后: 2024-01-15
={{ $today.minus({ days: 30 }).toISODate() }}         // 30 天前: 2023-12-08
={{ $today.plus({ months: 1 }).toISODate() }}         // 1 月后
={{ $today.plus({ hours: 8 }).toISO() }}              // 8 小时后（含时间）
```
...
={{ $now.setZone('Asia/Shanghai').toISODate() }}
```

**UTC 与本地互转**：
```
={{ $now.toUTC().toISO() }}                           // 转 UTC
={{ DateTime.fromISO($json.utc_time).setZone('Asia/Shanghai').toFormat('yyyy-MM-dd HH:mm') }}
```
...
**判断是否在某日期之后**：
```
={{ DateTime.fromISO($json.deadline) > $now }}        // 返回 true/false
={{ DateTime.fromISO($json.created_at).diffNow('days').days }}  // 距今天数（负数=过去）
```
...
**常用格式**：
```
={{ $now.toFormat('yyyy-MM-dd') }}                    // 2024-01-08
={{ $now.toFormat('yyyy年MM月dd日') }}                 // 2024年01月08日
={{ $now.toFormat('HH:mm:ss') }}                      // 14:30:00
={{ $now.toFormat('yyyy-MM-dd_HH-mm') }}              // 用于文件名: 2024-01-08_14-30
={{ $now.toMillis() }}                                // Unix 毫秒时间戳
={{ Math.floor($now.toMillis() / 1000) }}             // Unix 秒时间戳
```

### Week / Quarter

```
={{ $today.weekNumber }}                              // 第几周: 2
={{ $today.quarter }}                                 // 第几季度: 1
={{ $today.weekdayLong }}                             // 星期几: Monday
={{ $today.startOf('week').toISODate() }}              // 本周一
={{ $today.endOf('month').toISODate() }}               // 月末
```
