...
## Node Default Versions (n8n 2.x)

创建新节点时使用 defaultVersion。通过 `get_node` 可获取实例支持的最新版本。

| 节点 | nodeType | defaultVersion | 版本列表 | 关键变更 |
|------|----------|----------------|----------|----------|
| Form Trigger | `formTrigger` | **2.5** | 2, 2.1, 2.2, 2.3, 2.4, 2.5 | v2.4 fieldName 分离，v2.5 formFieldsDynamic |
| Form (Ending) | `form` | **2.5** | 1, 2.3, 2.4, 2.5 | 与 FormTrigger 同步，v2.3+ customCss |
| HTTP Request | `httpRequest` | **4.4** | 1…4.4 | v4.3 查询参数合并，v4.4 跨域凭证 |
| IF | `if` | **2.3** | 1, 2, 2.1, 2.2, 2.3 | v2.3 Filter v3 引擎 |
| Switch | `switch` | **3.4** | 1…3.4 | v3.3+ 动态表达式输出数 |
| Code | `code` | **2** | 1, 2 | v2 支持 Python（需 Task Runner） |
| Webhook | `webhook` | **2.1** | 1, 1.1, 2, 2.1 | — |
| Set | `set` | **3.4** | 1, 2, 3…3.4 | v3 声明式字段编辑器 |
| Merge | `merge` | **3.2** | 1, 2, 2.1, 3…3.2 | v3 合并模式重构 |
| Schedule Trigger | `scheduleTrigger` | **1.3** | 1, 1.1, 1.2, 1.3 | v1.2 增强 Cron |
| Execute Workflow | `executeWorkflow` | **1.3** | 1, 1.1, 1.2, 1.3 | v1.2 workflowInputs |
| Respond to Webhook | `respondToWebhook` | **1.5** | 1…1.5 | v1.4 configuredOutputs |

> **升级 n8n 后**：用 `get_node({mode: "versions"})` 检查各节点是否有新版本，同步更新此表和 reference 文档。
