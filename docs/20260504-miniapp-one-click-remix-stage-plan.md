...
- 智能复刻只选择产品，不导入人物/角色参考，避免后续生图触发敏感。
- 点击立即生成后立即在作品列表生成卡片，并进入可承载三阶段的爆款复刻详情页。

## 范围
- 小程序：`remix-generate` 页面上传、选择产品和提交元数据。
- 小程序：`storyboard-board` 页面按爆款复刻任务展示三阶段；一键生成是「爆款拆解 / 产品替换 / 视频生成」。
...
- n8n：一键生成复用 `viral_clone` 对应爆款拆解工作流。后续产品替换走现有分镜生图接口，但只传产品图和分镜参考帧，不传人物参考图。
...
- 产品第一个卡片为「不使用产品」，添加按钮在产品标题右上角；页面不再出现角色选择和复刻策略切换。
