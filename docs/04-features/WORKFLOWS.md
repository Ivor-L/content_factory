...
| 风格预设分析 | `xhs_vision_style_web` | `3pxJxnlNm5FNS7Ph` | 是 | `flow_xhs_Vision` | 小红书视觉风格分析 | 资产库上传风格预设后自动触发 | 下载参考图并生成 Style DNA / Prompt Kit / 版式蓝图，写回 `style_presets` 并扣减积分 |
...
| 正文转分镜（内容创作） | `xhs_chuangzuo_web` | `TPS71ueJGJKkEefz` | 是 | `flow_xhs_chuangzuo` | 正文转分镜 / Draft → Storyboard | 内容创作页“生成分镜板视频” | 解析脚本文案生成分镜提示词并写回分镜板 |
...
### 风格预设分析（Style Preset DNA）

- 触发入口：资产库上传视觉风格参考图（`style_presets` 队列）
- 目的：拆出可迁移的 Style DNA / Prompt Kit / 版式蓝图，供 xhs-poster 与视觉中台复用
- feature_key：`xhs_vision_style_web`
- workflow_id：`flow_xhs_Vision`
- n8n workflow id：`3pxJxnlNm5FNS7Ph`
- Prompt 模板：`docs/prompts/style-preset-analysis.md`
- 回写方式：n8n 异步完成后回调 `/api/webhook/style-analysis`，由 `app/api/webhook/style-analysis/route.ts` 落库并更新 spec / metadata

> **提示词结构**：为了确保风格拆解维度足够细，prompt 现已标准化到 `docs/prompts/style-preset-analysis.md`，覆盖 Style DNA、版式蓝图、生成 prompt、content mapping 四大块，并严格要求 JSON Schema。更新 n8n 逻辑时请以该文件为唯一准绳，避免 freestyle 修改导致字段缺失。

### 脚本拆解（Script Breakdown / Script Extract）

- 触发入口：脚本库「新增脚本」（`ScriptForm` 提交）
- 目的：把脚本拆成镜头/段落/台词/时长/节奏等结构化数据，供后续成片使用
- 是否异步：是（n8n 完成后回调 App）

#### 关键机制：脚本用途（scriptPurpose）决定走哪条工作流

`ScriptForm` 提交时用户必须选择「脚本用途」，**不同用途路由到完全不同的 n8n 工作流和回调接口**：
...
```
用户在 ScriptForm 选择脚本用途 → 提交
  ↓
createScript()  [app/(main)/scripts/actions.ts]
  ↓  落库
prisma.script.create()
  ↓  立即 fire-and-forget 触发（不阻塞返回）
...
- `one-click` / `extract-copy` 回调：`app/api/webhook/replication/script/route.ts` → 更新 `Script` 表的 `breakdown` / `blueprint` 字段
- `storyboard` 回调：`app/api/webhook/storyboard-breakdown/route.ts` → 更新 `StoryboardTask` 或 `Script` 表（兼容两种 task_id 类型）

#### 注意事项（避免踩坑）

1. **`/api/scripts` POST 接口不传 `scriptPurpose`**，始终默认走 `one-click` 工作流。如果将来有其他入口通过该 API 创建脚本，需在调用方显式传入 `scriptPurpose`。
2. **拆解是 fire-and-forget**：`createScript()` 不等待拆解完成即返回成功。拆解失败只记录日志，不影响脚本创建，前端需通过状态轮询感知结果。
3. **拆解完成后不会自动发起成片**：`ReplicationForm`（一键成片/分镜成片）提交时读取已拆解的 `script.breakdown` / `script.blueprint` 字段，用户需在拆解完成后手动触发成片。
4. **回调依赖 `ADMIN_TOKEN`**：两条回调路径均使用 `isValidAdminWebhookRequest()` 鉴权，本地和生产都需在环境变量中配置（见 `ENV_AND_SECRETS.md` 第 3.3 节）。

#### 代码位置索引

| 模块 | 文件 |
|------|------|
| 表单 UI + 用途选择 | `components/ScriptForm.tsx` |
| Server Action（创建+触发） | `app/(main)/scripts/actions.ts` |
| n8n 调用封装（路由逻辑） | `lib/n8n.ts → breakdownScript()` |
| 一键复刻回调处理 | `app/api/webhook/replication/script/route.ts` |
| 分镜复刻回调处理 | `app/api/webhook/storyboard-breakdown/route.ts` |
| 回调鉴权 | `lib/webhookAuth.ts` |

- workflow_id（积分系统，one-click）：`flow_script_dna`
- n8n workflow id（one-click）：`xXldwYS5d3lCTNwE`
- workflow_id（积分系统，storyboard）：`flow_storyboard_disassembly`
- n8n workflow id（storyboard）：`QdlJ7C7hyTtjy0eH`
...
- 触发入口：内容创作面板「生成分镜板视频」动作
- 目的：将创作稿件（正文+标题）拆解为分镜脚本、图像提示词、视频提示词并写回分镜板
- feature_key：`xhs_chuangzuo_web`
- workflow_id（积分系统对应）：`flow_xhs_chuangzuo`
- n8n workflow id：`TPS71ueJGJKkEefz`
- 是否异步：是（n8n 完成解析后回调 `/api/webhook/storyboard-content`）
- 关键入参：
  - `script`：创作稿正文（必填）
  - `title`：稿件标题（可选）
  - `creative_task_id`：来源任务 ID（可选，用于链路追踪）
  - `api_key`：用户积分凭据
  - `callback_url`：回写地址，使用 `APP_URL/api/webhook/storyboard-content`
- 回写内容：每个分镜的 `idx/口播文案/图片提示词/视频提示词`，系统落库到 `storyboard_tasks` + `storyboard_segments`
