        ...
        messages: [
          {
            ...
            content: [
              ...
              "渲染器支持：# / ## / ### 标题、段落、引用 >、无序/有序列表、**加粗**、==高亮重点==、++下划线重点++、''标记重点''、~~删除线~~、分隔线 ---、Markdown 表格。",
              "硬性要求：",
              ...
              "5) 重点样式要更丰富：核心结论优先用 ==高亮重点==；行动/方法用 ++下划线重点++；短金句或转折点用 ''标记重点''；关键词或数字也可用 **加粗**。被标记的文字必须来自原文。",
              "6) 可以把原文里天然并列、对比、参数、费用、步骤、优缺点等内容整理成表格；表头必须来自原文已有分类/字段，没有现成字段就不要造表。",
              "7) 可以修正错误换行、合并被断开的句子、拆分过长段落，让阅读节奏更清晰。",
              "8) 当原文有结论句、提醒句或口号句时，优先保留为引用块，例如：> 先跑通，再放大。",
              "9) 同一段内容不要堆叠超过 2 种重点标记；不要为了凑样式而标记无关文字。",
              "10) 不要输出“封面标题/副标题/正文/标签”等模板字段，除非原文已经有这些字。",
              "11) 输出严格 JSON，不要 markdown 代码块。",
            ].join("\n"),
          },
          {
            role: "user",
