附录：《万字讲透Claude Code从"能用"到"真好用"的分水岭：Workspace 深度解析》
...
你的项目文件夹/
├── CLAUDE.md ← 工作手册（最重要的文件）
├── .claudeignore ←告诉Claude 哪些文件别碰
└── .claude/
    ├──settings.json ← 权限配置
    ├── mcp-servers.json ← 外部工具配置
    ├──rules/← 分模块的规则
    ├──skills/← 技能包
    └──commands/← 自定义命令
...
├── skills/← 全局技能包
└── projects/<项目路径>/memory/ ← Claude 自己记的笔记
...
第四步：测试和迭代
Skill 写好了，试用几次。看看 Claude 触发的时机对不对（不对就调 description）、执行步骤有没有遗漏（有就补）、有没有新的坑需要记录（有就加）。好的 Skill 不是一次写完的，是用出来的。
第五步：决定放哪里
...
Claude Code 总共 200K 的上下文窗口，系统占 15-20K，CLAUDE.md 和 Memory 占 5-10K。装五六个 MCP，光工具描述可能吃掉 60-80K——三分之一没了。
所以原则是：用什么装什么，不用的及时删。

 MCP 和 Skills 的关系
MCP 解决"能不能做"，Skills 解决"做得好不好"。
只装 MCP 不写 Skill，Claude 能调工具但不知道正确的工作流程。MCP + Skill 配合，才是稳定输出的关键。
...
更厉害的是 PreToolUse Hook，它可以在 Claude 执行操作之前先做检查。相当于给 Claude 装了一道安全门。

📘 怎么用效果最好
Hooks 配置写在 settings.json 里。
日常最实用的就两个：PostToolUse（工具调用后自动执行）和 PreToolUse（工具调用前做检查/拦截）。
最推荐的第一个 Hook：自动 Git 存档。 
配上之后，Claude 每做一步操作（改文件、跑命令），都会自动 commit 一个 checkpoint。随时回滚到任意一步。
第二个推荐：自动格式化。 
...
这使得 Claude 会越用越顺手。 它记住了你的习惯，记住了项目的坑，不需要你反复教。
...
如果在 CI 环境不需要 Memory，设环境变量：CLAUDE_CODE_DISABLE_AUTO_MEMORY=1 就行。