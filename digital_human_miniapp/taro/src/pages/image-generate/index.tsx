import { View, Text, ScrollView, Textarea, Image, Input, RichText } from '@tarojs/components';
...
const CARD_LAYOUT_STYLES: CardStylePreset[] = [
  {
    id: 'cinematic-film',
    title: '电影胶片',
    desc: '暗场电影感，适合叙事与情绪长文',
    defaultMarkdown: `# 复盘：内容增长不是玄学
...
export default function ImageGeneratePage() {
  ...
  const cardPreviewHtml = useMemo(() => {
    const source = cardMarkdown.trim();
    if (!source) {
      return renderMiniMarkdown('# 标题示例\n\n支持下划线 ++重点++ 和 ==高亮==');
    }
  ...
  const handleCreateCard = async () => {
    ...
    try {
      ...
      const styleMapping: Record<CardStyleId, string> = {
        'cinematic-film': 'dark',
        'starry-night': 'dark',
        polaroid: 'clean',
        'notion-style': 'clean',
          ...
          <View className='panel'>
            <View className='card-topbar'>
              ...
              <View className='card-setting-trigger' onClick={() => setCardSettingsOpen((prev) => !prev)}>
                <Text className='card-setting-trigger-text'>{cardSettingsOpen ? '收起设置' : '设置'}</Text>
              </View>
            </View>
            ...
            {cardSettingsOpen && (
              <>
                ...
                <View className='card-style-row'>
                  {CARD_LAYOUT_STYLES.map((item) => {
                    const active = selectedCardStyle === item.id;
                    return (
                      <View
                        key={item.id}
                        className={`style-chip style-chip--block ${active ? 'style-chip--active' : ''}`}
                        onClick={() => {
                      ...
                      >
                        <Text className={`style-chip-text ${active ? 'style-chip-text--active' : ''}`}>{item.title}</Text>
                        <Text className={`style-chip-sub ${active ? 'style-chip-sub--active' : ''}`}>{item.desc}</Text>
                      </View>
                    );
                  })}
                </View>
                  ...
                  {cardIncludeCover && (
                    <View className='card-cover-config'>
                      <View className='card-config-row card-config-row--no-divider'>
                        ...
                        <View className='cover-style-grid'>
                          {CARD_COVER_STYLE_OPTIONS.map((item) => (
                            <View
                              key={item.id}
                              className={`cover-style-card ${cardCoverStyleId === item.id ? 'cover-style-card--active' : ''}`}
                              onClick={() => setCardCoverStyleId(item.id)}
                            >
                              <View className='cover-style-preview' style={{ background: item.preview }} />
                ...
                <View className='info-input-box'>
                  ...
                  <View className='info-input-actions'>
                    <View
                      ...
                      onClick={() => {
                        setCardMarkdown(selectedCardStylePreset.defaultMarkdown);
                        Taro.showToast({ title: '已恢复默认样式内容', icon: 'none' });
                      }}
                    >
                      <Text className='input-action-btn-text input-action-btn-text--ghost'>默认内容</Text>
                    </View>
                    <View className='input-action-btn' onClick={handleFindInspiration}>
                      <Text className='input-action-btn-text'>没有文案？去找灵感</Text>
                    </View>
            ...
            {cardEditorMode === 'preview' && (
              <>
                ...
                <View className={`preview-card ${cardPreviewThemeClass}`}>
                  <RichText className='preview-richtext' nodes={cardPreviewHtml} />
                </View>
              </>
            )}
