        ...
        {!loading && !errorText && !!config && (
          <>
            ...
            <ScrollView scrollY className='monetization-content-scroll'>
              <View className='monetization-grid'>
                {(activeCategory?.items || []).map((item) => (
                  <View key={item.id} className='monetization-card' onClick={() => handleOpenItem(item)}>
                    <View className='monetization-card-media'>
                      {!!item.coverImageUrl ? (
                        <Image className='monetization-card-cover' src={item.coverImageUrl} mode='aspectFill' />
                      ) : (
                        <View className='monetization-card-fallback'>
                          <Text className='monetization-card-fallback-text'>{item.title.slice(0, 4)}</Text>
                        </View>
                      )}
                    </View>
                    <View className='monetization-card-body'>
                      <Text className='monetization-card-title'>{item.title}</Text>
                      {!!item.subtitle && <Text className='monetization-card-desc'>{item.subtitle}</Text>}
                      <View className='monetization-card-footer'>
                        <View className='monetization-card-tags'>
                          {(item.tags || []).slice(0, 2).map((tag) => (
                            <Text key={tag} className='monetization-tag'>{tag}</Text>
                          ))}
                        </View>
