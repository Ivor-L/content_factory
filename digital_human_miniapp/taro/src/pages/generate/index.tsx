...
export default function GeneratePage() {
  ...
  const handleSubmitSkeletonStoryboard = async () => {
    ...
    try {
      ...
      setSkeletonScript('');
      Taro.navigateTo({
        url: `/pages/storyboard-board/index?id=${encodeURIComponent(result.taskId)}&title=${encodeURIComponent('3D骨骼分镜板')}`,
      });
    } catch (err) {
              ...
              <View className='section'>
                ...
                <View className='script-input-box'>
                  <Textarea
                    ...
                    maxlength={500}
                  />
                  <View className='script-input-actions'>
                    <View className='script-action-btn' onClick={handleFindInspiration}>
                      <Text className='script-action-btn-text'>没有文案？去找灵感</Text>
                    </View>
                    <View className='script-action-btn script-action-btn--ghost' onClick={handlePasteScript}>
                      <Text className='script-action-btn-text script-action-btn-text--ghost'>粘贴</Text>
                    </View>
                  </View>
                </View>
