...
function getHotStats(item: any, rawPayload?: Record<string, unknown> | null) {
  ...
  return {
    likes: pickNumber(stats.likes, stats.likeCount, stats.like_count, stats.likedCount, stats.liked_count, rawStats.likes, rawStats.likeCount, rawStats.like_count, rawStats.liked_count, rawPayload?.likes, rawPayload?.likeCount, rawPayload?.like_count, rawPayload?.liked_count, rawPayload?.点赞数, rawPayload?.点赞, rawPayload?.点赞数量) ?? pickNumberByKeys(objects, ['点赞数', '点赞', '点赞数量', '赞数', 'liked_count', 'like_count', 'likeCount', 'likedCount', 'likes', 'interact_info', 'interactInfo', 'interactionInfo']),
    collects: pickNumber(stats.collects, stats.collectCount, stats.collect_count, stats.collectedCount, stats.collected_count, rawStats.collects, rawStats.collectCount, rawStats.collect_count, rawStats.collected_count, rawPayload?.collects, rawPayload?.collectCount, rawPayload?.collect_count, rawPayload?.collected_count, rawPayload?.收藏数, rawPayload?.收藏, rawPayload?.收藏数量) ?? pickNumberByKeys(objects, ['收藏数', '收藏', '收藏数量', 'collected_count', 'collect_count', 'collectCount', 'collectedCount', 'collects', 'interact_info', 'interactInfo', 'interactionInfo']),
    comments: pickNumber(stats.comments, stats.commentCount, stats.comment_count, rawStats.comments, rawStats.commentCount, rawStats.comment_count, rawPayload?.comments, rawPayload?.commentCount, rawPayload?.comment_count, rawPayload?.评论数, rawPayload?.评论, rawPayload?.评论数量) ?? pickNumberByKeys(objects, ['评论数', '评论', '评论数量', 'comment_count', 'commentCount', 'comments', 'interact_info', 'interactInfo', 'interactionInfo']),
    shares: pickNumber(stats.shares, stats.shareCount, stats.share_count, rawStats.shares, rawStats.shareCount, rawStats.share_count, rawPayload?.shares, rawPayload?.shareCount, rawPayload?.share_count, rawPayload?.分享数, rawPayload?.分享, rawPayload?.分享数量) ?? pickNumberByKeys(objects, ['分享数', '分享', '分享数量', 'share_count', 'shareCount', 'shares', 'interact_info', 'interactInfo', 'interactionInfo']),
  };
}
...
function normalizeHotMediaUrls(item: any): string[] | null {
  ...
  const candidates: unknown[] = [
    ...
    rawPayload?.cover,
    rawPayload?.poster,
    rawPayload?.posterUrl,
    rawPayload?.poster_url,
    rawPayload?.displayUrl,
    rawPayload?.display_url,
    rawPayload?.thumb,
    rawPayload?.thumbUrl,
    rawPayload?.thumb_url,
    ...
    parseObject(rawPayload?.note)?.poster,
    parseObject(rawPayload?.note)?.posterUrl,
    parseObject(rawPayload?.note)?.poster_url,
    parseObject(rawPayload?.note)?.displayUrl,
    parseObject(rawPayload?.note)?.display_url,
    ...
    parseObject(rawPayload?.data)?.poster,
    parseObject(rawPayload?.data)?.posterUrl,
    parseObject(rawPayload?.data)?.poster_url,
    parseObject(rawPayload?.data)?.displayUrl,
    parseObject(rawPayload?.data)?.display_url,
